...
| `multi_host` | No | `false` | If `true`, isolates benchmark output writes into a unique folder per job on shared multi-host volumes. |
| `multi_host_shared_dir` | No | `/tmp/bap-ml-actions-ci` | Path to the shared volume mounted across multi-host runners. |
...
The reusable workflow injects three standard environment variables into your workload's execution environment to handle outputs:
...
### Saving Dynamic Metadata

If your workload wants to record extra information that should be attached to the final `BenchmarkResult` (e.g., driver versions, specific hardware properties detected at runtime), save them as JSON files in `WORKLOAD_METADATA_DIR`.

The platform will automatically parse all JSON objects in this directory and merge them into the result's metadata map.

#### Merging Priority

If the same metadata key is provided by multiple sources, the platform resolves conflicts using the following priority (highest to lowest):

1.  **Dynamic Metadata**: Discovered by the workload script at runtime (e.g., hardware IDs).
2.  **Workflow Input**: Supplied via the `custom_metadata` input when triggering the GitHub Action.
3.  **Static Metadata**: Declared in the benchmark registry (`.pbtxt` file).
...
if metadata_dir:
  ...
  metadata = {"cuda_version": "12.2", "driver_version": "535.100"}
